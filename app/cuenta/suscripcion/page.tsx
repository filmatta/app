...
export default async function SubscriptionPage({ searchParams }: {
  ...
  return <main className="min-h-screen bg-[#080808] px-6 py-12 text-white">
    <div className="mx-auto max-w-3xl">
      ...
      {feedback.error === "country" && <p role="alert" className="mt-5 text-red-200">Las suscripciones están limitadas a México. Selecciona México como país de facturación.</p>}
      {feedback.error === "checkout" && <p role="alert" className="mt-5 text-red-200">No pudimos iniciar Checkout. Revisa tu suscripción existente o intenta más tarde.</p>}
      {(result.error || invoiceResult.error || (feedback.error && !["country", "checkout"].includes(feedback.error))) && <p role="alert" className="mt-5 text-red-200">No pudimos cargar toda la información de Billing. Intenta más tarde.</p>}
