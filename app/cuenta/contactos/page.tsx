...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<{ box?: string; page?: string; history?: string }> }) {
  ...
  return <div className="editorial-page"><SiteHeader /><main className="network-shell"><header className="network-heading"><p className="eyebrow">TU ESPACIO PRIVADO</p><h1>{box === "accepted" ? "Contactos" : "Solicitudes"}</h1><h2 className="network-credit-title">Créditos de contacto</h2>
    {w && <div className="network-credit-metrics">{w.is_pro ? <><StatusBadge tone="pro">PRO</StatusBadge><span>Sin límite de créditos. Se mantienen límites de envío.</span></> : <><StatusBadge tone="success">{w.remaining_contacts} {w.remaining_contacts === 1 ? "disponible" : "disponibles"}</StatusBadge><StatusBadge tone="warning">{w.reserved_contacts} {w.reserved_contacts === 1 ? "reservado" : "reservados"}</StatusBadge><StatusBadge>{w.consumed_contacts} {w.consumed_contacts === 1 ? "consumido" : "consumidos"}</StatusBadge></>}</div>}<p className="network-credit-note"><CreditIcon />Los créditos se gastan cuando este perfil acepta tu solicitud.</p></header>
    <nav className="network-tabs" aria-label="Tipo de solicitudes"><Link href="/cuenta/contactos" aria-current="page">Profesionales</Link><Link href="/cuenta/contactos/locaciones">Locaciones</Link></nav>
