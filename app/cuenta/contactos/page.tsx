...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<{ box?: string; page?: string; history?: string }> }) {
  ...
  const [list,wallet] = await Promise.all([box === "legacy" ? db.rpc("list_my_legacy_profile_contacts", { p_box: history, p_page: page }) : db.rpc("list_my_contact_requests", { p_box: box, p_page: page }),db.rpc("get_my_contact_wallet")]);
