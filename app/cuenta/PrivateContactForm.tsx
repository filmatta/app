...
export default function PrivateContactForm({ initial, profileEditor = false }: { initial: PrivateContact; profileEditor?: boolean }) {
  ...
  return <form className="private-contact-form" onSubmit={async e => {
    ...
    {wa?.canonical && <><p>Formato internacional: <strong>{wa.display}</strong></p>{profileEditor ? <SelectionRow checked={confirmed} onChange={e => setConfirmed(e.target.checked)}>Confirmo que éste es el número que quiero guardar.</SelectionRow> : <label className="private-contact-check"><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />Confirmo que éste es el número que quiero guardar.</label>}</>}
    ...
    <fieldset><legend>Al aceptar una solicitud, compartir:</legend>
      {([["share_instagram", "Instagram"], ["share_whatsapp", "WhatsApp"], ["share_email", "Email"], ["share_phone", "Teléfono"]] as const).map(([key,label]) => profileEditor ? <SelectionRow key={key} checked={channels[key]} onChange={e => setChannels({ ...channels, [key]: e.target.checked })}>{label}</SelectionRow> : <label key={key} className="private-contact-check"><input type="checkbox" checked={channels[key]} onChange={e => setChannels({ ...channels, [key]: e.target.checked })} />{label}</label>)}
    </fieldset>
