...
import LocationOwnerEditorShell, { LocationSectionForm } from "./LocationOwnerEditorShell";
...
export default function LocationOwnerEditor({
  ...
  return <LocationOwnerEditorShell currentStatus={location.status} statusAction={statusAction}><div className="mt-10">
    ...
    <div className="divide-y divide-white/10">
      <EditorSection section="tour" title="Recorrido" actionLabel={tour?.isActive ? "Volver a grabar" : "Grabar recorrido"} complete={progress.items[9].complete} optional open={activeSection === "tour"} summary={tour?.isActive || location.tour_video_url ? "Recorrido disponible" : "Muestra el espacio en una toma continua desde tu teléfono."}>
      ...
      <EditorSection section="photos" title="Fotos del lugar" actionLabel={readyPhotos.length ? "Administrar fotos" : "Agregar fotos y elegir portada"} complete={progress.items[5].complete} open={activeSection === "photos"} summary={readyPhotos.length ? `${readyPhotos.length} foto${readyPhotos.length === 1 ? " lista" : "s listas"}` : "Añade fotos para mostrar la distribución y los accesos."}>
        <LocationPhotoManager locationId={location.id} photos={photos} />
      </EditorSection>

      <EditorSection section="identity" title="Información" actionLabel="Editar información" complete={progress.items[0].complete} open={activeSection === "identity"}>
        <LocationSectionForm section="identity" action={actions.identity} className="space-y-5"><LocationIdentityFields defaultTitle={location.title} defaultSlug={location.slug} autoGenerateSlug={false} /><Field label="Tipo de espacio"><input name="space_type" required maxLength={120} defaultValue={location.space_type} className={inputClass} /></Field><Field label="Entorno"><select name="environment" defaultValue={location.environment} className={inputClass}>{LOCATION_ENVIRONMENTS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></Field><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="location" title="Ubicación" actionLabel={location.geography_source ? "Editar ubicación" : "Revisar ubicación"} complete={progress.items[1].complete} open={activeSection === "location"} summary={`${location.region_name ? `${location.region_name} · ` : ""}${location.municipality_name ? `${location.municipality_name} · ` : ""}${location.city}`}>
        <LocationSectionForm section="location" action={actions.location} className="space-y-5"><LocationGeographyFields initial={{ countryCode: location.country_code, regionCode: location.region_code, municipalityCode: location.municipality_code, localityCode: location.locality_code, postalCode: location.postal_code, city: location.city, area: location.area }} /><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="pricing" title="Capacidad y tarifas" actionLabel="Editar capacidad y tarifas" complete={progress.items[2].complete && progress.items[3].complete} open={activeSection === "pricing"} summary={pricingSummary(location)}>
        <LocationSectionForm section="pricing" action={actions.pricing} className="space-y-6"><LocationPricingEditor initialCapacity={typeof capacity === "number" ? capacity : null} initialMode={location.rate_mode} initialTiers={location.rate_tiers} initialMinimumHours={location.minimum_hours === null ? null : Number(location.minimum_hours)} legacyPrice={formatLegacyLocationPrice({ priceAmount: numberOrNull(location.price_amount), priceCurrency: location.price_currency, priceUnit: location.price_unit })} /><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="description" title="Descripción" actionLabel={progress.items[4].complete ? "Editar descripción" : "Describe tu espacio"} complete={progress.items[4].complete} open={activeSection === "description"} summary={location.summary || location.description || "Explica la distribución, los accesos y qué hace especial al espacio."}>
        <LocationSectionForm section="description" action={actions.description} className="space-y-5"><Field label="Descripción corta"><textarea name="summary" maxLength={500} rows={3} defaultValue={location.summary ?? ""} className={inputClass} /></Field><Field label="Descripción completa"><textarea name="description" maxLength={20_000} rows={7} defaultValue={location.description ?? ""} className={inputClass} /></Field><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="characteristics" title="Características" actionLabel={progress.items[6].complete ? "Editar características" : "Completar características"} complete={progress.items[6].complete} open={activeSection === "characteristics"} summary={progress.items[6].complete ? `${Object.keys(location.characteristics).length - 1} datos adicionales` : "Superficie, luz, accesos, servicios y equipamiento."}>
        <LocationSectionForm section="characteristics" action={actions.characteristics} className="space-y-6"><LocationCharacteristicsEditor value={location.characteristics} /><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="conditions" title="Condiciones de rodaje" actionLabel={progress.items[7].complete ? "Editar condiciones" : "Completar condiciones de rodaje"} complete={progress.items[7].complete} open={activeSection === "conditions"} summary={progress.items[7].complete ? `${Object.keys(location.shooting_conditions).length} condiciones especificadas` : "Horarios pendientes · Equipo y montaje sin completar"}>
        <LocationSectionForm section="conditions" action={actions.conditions} className="space-y-6"><p className="text-sm leading-6 text-white/45">{LOCATION_CONDITIONS_NOTICE}</p><LocationConditionsEditor initialValue={location.shooting_conditions} /><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="notes" title="Restricciones y operación" actionLabel="Editar condiciones operativas" complete={Boolean(location.restrictions || location.operational_notes)} open={activeSection === "notes"} summary={location.restrictions || location.operational_notes || "Horarios, visita técnica, depósito, seguro o limpieza."}>
        <LocationSectionForm section="notes" action={actions.notes} className="space-y-5"><Field label="Restricciones básicas"><textarea name="restrictions" maxLength={10_000} rows={5} defaultValue={location.restrictions ?? ""} className={inputClass} /></Field><Field label="Tarifa y condiciones operativas"><textarea name="operational_notes" maxLength={5_000} rows={5} defaultValue={location.operational_notes ?? ""} className={inputClass} /></Field><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>

      <EditorSection section="contact" title="Contacto" actionLabel={progress.items[8].complete ? "Configurar contacto" : "Configurar contacto"} complete={progress.items[8].complete} open={activeSection === "contact"} summary={progress.items[8].complete ? "Canales privados configurados" : "Los datos sólo se comparten cuando aceptas una solicitud."}>
        <LocationSectionForm section="contact" action={actions.contact} className="space-y-5"><div className="grid gap-5 sm:grid-cols-2"><Field label="Email"><input name="contact_email" type="email" maxLength={254} defaultValue={contact?.email ?? ""} className={inputClass} /></Field><Field label="Teléfono"><input name="contact_phone" type="tel" maxLength={40} defaultValue={contact?.phone ?? ""} className={inputClass} /></Field><Field label="WhatsApp"><input name="contact_whatsapp" maxLength={20} defaultValue={contact?.whatsapp ?? ""} className={inputClass} /></Field><Field label="Instagram"><input name="contact_instagram" maxLength={30} defaultValue={contact?.instagram ?? ""} className={inputClass} /></Field></div><input type="hidden" name="contact_website" value={contact?.website ?? ""} /><LocationSectionSubmit /></LocationSectionForm>
      </EditorSection>
    </div>
  </div></LocationOwnerEditorShell>;
}

function EditorSection({ section, title, actionLabel, complete, optional = false, summary, open, children }: { section: LocationEditorSection | "tour" | "photos"; title: string; actionLabel: string; complete: boolean; optional?: boolean; summary?: string; open?: boolean; children: React.ReactNode }) {
  return <section id={`location-section-${section}`} className="scroll-mt-24 py-8"><div className="grid gap-4 md:grid-cols-[220px_1fr]"><div><h3 className="font-semibold">{title}</h3><span className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-[11px] ${complete ? "bg-emerald-300/10 text-emerald-200" : "bg-white/[0.06] text-white/45"}`}>{complete ? "Completo" : optional ? "Opcional" : "Pendiente"}</span></div><div><p className="max-w-2xl text-sm leading-6 text-white/50">{summary}</p><details open={open} className="group mt-4 rounded-2xl border border-white/10 bg-white/[0.02] p-5"><summary className="cursor-pointer list-none text-sm font-semibold text-white/80 group-open:mb-6">{actionLabel} <span aria-hidden="true" className="ml-2 text-white/35">＋</span></summary>{children}</details></div></div></section>;
}
