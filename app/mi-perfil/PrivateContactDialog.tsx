...
export default function PrivateContactDialog({ close }: { close: () => void }) {
  ...
  return <EditorDialog title="Datos de contacto" close={close} busy={false}>{value ? <PrivateContactForm initial={value} /> : error ? <p role="alert">{error}</p> : <p role="status">Cargando datos de contacto…</p>}</EditorDialog>;
}
