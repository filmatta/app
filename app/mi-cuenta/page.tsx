...
export default async function AccountDashboardPage() {
 const viewer=await getViewer();if(!viewer)redirect("/login?next=%2Fmi-cuenta");
 ...
 return <div className="editorial-page"><SiteHeader/><main className="network-shell account-dashboard">
 <header className="account-dashboard-header"><p className="eyebrow">MI CUENTA</p><h1>Hola, {viewer.fullName || "bienvenido"}.</h1><p>Aquí puedes continuar tu trabajo y administrar tu cuenta.</p></header>
 ...
 <Link className="network-card account-quick-card" href="/mi-perfil"><div className="account-quick-icon">{portrait?.portrait_media_id||portrait?.portrait_url?<ProfileAvatar id={portrait.portrait_media_id} fallbackUrl={portrait.portrait_url} name={d.profile?.display_name||viewer.displayName}/>:<Icon kind="profile"/>}</div><h3>Mi perfil</h3>{d.profile?<StatusBadge tone={d.profile.is_public?"success":"neutral"}>{d.profile.is_public?"Publicado":"Borrador"}</StatusBadge>:d.profile===null?<p>Crea tu presencia profesional.</p>:null}<span className="network-card-cta">Editar perfil →</span></Link>
 ...
 <Link className="network-card account-quick-card" href="/cuenta/contactos"><div className="account-quick-icon"><Icon kind="request"/></div><h3>Solicitudes / Contactos</h3>{d.summary&&(d.summary.pending_received?<StatusBadge tone="warning">{d.summary.pending_received} pendientes</StatusBadge>:<p>No tienes solicitudes pendientes.</p>)}<span className="network-card-cta">Ver solicitudes →</span></Link>
 ...
 <Link className="network-card account-quick-card" href="/cuenta#mis-cursos"><div className="account-quick-icon"><Icon kind="learn"/></div><h3>Learn</h3><p>Tu biblioteca y acceso al catálogo.</p><span className="network-card-cta">Continuar aprendiendo →</span></Link>
 </div></section>
 ...
 <section className="account-configuration" aria-labelledby="account-settings"><h2 id="account-settings">Configuración</h2><div className="account-settings-links"><Link href="/cuenta#configuracion">Cuenta →</Link><Link href="/cuenta#datos-contacto">Datos de contacto →</Link><Link href="/cuenta#seguridad">Seguridad →</Link><Link href="/cuenta/suscripcion">Plan / suscripción →</Link></div>{billing.plan&&<p className="account-plan">Tu plan <StatusBadge tone={billing.plan==="pro"?"pro":"plus"}>{billing.plan}</StatusBadge></p>}</section>
 </main></div>;
}
