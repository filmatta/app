...
export function testConfiguration() {
  assert.equal(
    process.env.FILMATTA_RUN_REMOTE_TESTS,
    TEST_REF,
    "Explicit Test opt-in required.",
  );
  assert.ok(
    process.env.FILMATTA_TEST_ENV_FILE,
    "Supply the existing Test-only env file path.",
  );
  const allowed = new Set([
    "NEXT_PUBLIC_SUPABASE_URL",
    "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY",
    "SUPABASE_SERVICE_ROLE_KEY",
    "BILLING_TEST_SUPABASE_PROJECT_REF",
  ]);
  const config = {};
  for (const line of fs
    .readFileSync(process.env.FILMATTA_TEST_ENV_FILE, "utf8")
    .split(/\r?\n/)) {
    const match = line.match(/^\s*([A-Z0-9_]+)\s*=\s*(.*?)\s*$/);
    if (match && allowed.has(match[1]))
      config[match[1]] = match[2].replace(/^(['"])(.*)\1$/, "$2");
  }
  assert.equal(
  ...
  if (service.startsWith("eyJ"))
    assert.equal(
      JSON.parse(Buffer.from(service.split(".")[1], "base64url")).ref,
      TEST_REF,
      "Refuse mismatched service key.",
    );
  return config;
}
export async function withTestUsers(run) {
  ...
  try {
    for (const kind of ["owner", "stranger", "admin", "outsider"]) {
      const email = `${prefix}-${kind}@example.invalid`;
      const password = randomBytes(24).toString("base64url") + "aA1!";
      const fullName = `Integration ${kind} PrivateSurname`;
      const result = await adminSetup.auth.admin.createUser({
        email,
        password,
        email_confirm: true,
      ...
      const user = { id: result.data.user.id, email, fullName, kind };
      users.push(user);
      ...
      user.client = normal;
      if (kind === "admin") {
        const promoted = testSql(
          `update public.profiles set role='admin' where id='${user.id}' and exists(select 1 from auth.users where id='${user.id}' and email='${email}') returning id;`,
        );
