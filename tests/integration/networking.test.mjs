...
try {
 ...
 const pendingView=ok(await first.t.db.rpc('get_authorized_networking_project',{p_slug:before.slug}),'pending Project');assert.equal(pendingView.is_owner,false);
 ...
 const sharedProject=async actor=>ok(await actor.db.rpc('get_authorized_networking_project',{p_slug:before.slug}),'authorized Project');
 assert.equal((await sharedProject(free)).is_owner,true);
 assert.equal((await sharedProject(first.t)).is_owner,false);
 assert.equal((await sharedProject(first.t)).client_name,'');
 assert.equal(await sharedProject(pro),null);
 assert.ok((await anon.rpc('get_authorized_networking_project',{p_slug:before.slug})).error);
 const historical=(await read(first.t,first.r.data)).project_snapshot;
 ok(await free.db.rpc('save_my_networking_project',{p_id:project,p_data:{...projectData,title:'Estado actual QA',status:'archived',operational_status:'inactive',share_client_name:true}}),'shared archived Project');
 const current=await sharedProject(first.t);
 assert.equal(current.title,'Estado actual QA');assert.equal(current.client_name,'Cliente privado');assert.equal(current.status,'archived');assert.equal(current.operational_status,'inactive');
 assert.deepEqual((await read(first.t,first.r.data)).project_snapshot,historical);
 assert.equal((await read(first.t,first.r.data)).project_slug,before.slug);
 assert.equal(ok(await first.t.db.from('projects').select('*').eq('id',project),'raw project denied').length,0);
 assert.ok((await first.t.db.rpc('save_my_networking_project',{p_id:project,p_data:projectData})).error);
 console.log('PASS: shared Project accepted/live/archive, client consent, immutable snapshot, outsider/anon denial and owner-only writes.');
