...
test("anonymous navigation presents verticals; member navigation keeps direct access", () => {
  ...
  assert.equal(anon.find(item => item.label === "Learn").href, "/descubre/learn");
...
test("landings have real CTAs, complete content and no duplicated H1 values", () => {
  assert.equal(new Set(Object.values(landings).map(item => item.title)).size, Object.keys(landings).length);
  for (const landing of Object.values(landings)) {
    assert.equal(landing.capabilities.length, 3);
    assert.equal(landing.steps.length, 3);
    for (const { href } of [landing.primary, landing.secondary, landing.connection]) {
      if (href.startsWith('/descubre/')) assert.ok(getLanding(href.split('/').pop()));
      else assert.ok(fs.existsSync(`app${href}/page.tsx`), href);
    }
  }
});
test("only creation CTAs require registration and preserve next", () => {
  assert.equal(landingActionHref('/mi-perfil', false), '/registro?next=%2Fmi-perfil');
