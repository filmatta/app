...
function callbackWith(auth, allowAuthAttempt = async () => true) {
  return load("app/auth/callback/route.ts", {
    ...
    "@/lib/security/auth-rate-limit": { allowAuthAttempt },
    "@/lib/supabase/server": {
      async createClient() {
        return { auth };
      },
...
test("recovery token hash fails safely when callback attempts are limited", async () => {
  ...
  assert.equal(destination.pathname, "/recuperar-contrasena");
  assert.equal(destination.searchParams.get("next"), "/cuenta");
  assert.equal(destination.searchParams.get("error"), "invalid_recovery");
});
