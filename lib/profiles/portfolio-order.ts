...
export function portfolioGroups(items: MediaItem[]) {
  const ordered = items.filter(i => !i.purpose || i.purpose === "portfolio").sort((a, b) => Number(b.featured) - Number(a.featured) || a.sort_order - b.sort_order || a.id.localeCompare(b.id));
