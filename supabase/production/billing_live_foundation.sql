...
-- Do not run this script against the Billing Test project. It intentionally creates
-- Live-only billing tables, access RPCs, scheduled-cancellation fields, and Admin Grants.
-- Access starts disabled; set public.billing_settings.live_access_enabled only after
...
create table public.billing_settings (
  ...
  live_access_enabled boolean not null default false
);
...
language sql stable security definer set search_path = '' as $$
  ...
  where c.user_id = (select auth.uid()) and e.valid_until > now()
    and (select live_access_enabled from public.billing_settings where id)
...
language sql stable security definer set search_path = '' as $$
  with settings as (
    select coalesce(
      (select live_access_enabled from public.billing_settings where id),
      false
    ) as enabled
  ),
  stripe_access as (
