...
export default function NetworkMemberCard({ member: m, following }: { member: NetworkMember; following: boolean }) {
  ...
  return <article className="network-card"><header><div className="network-avatar">{m.portrait_media_id || m.portrait_url ? <ProfileAvatar id={m.portrait_media_id} fallbackUrl={m.portrait_url} name={m.display_name} /> : <span>{m.display_name[0]}</span>}</div><div><h2>{m.slug ? <Link href={`/perfiles/${m.slug}`}>{m.display_name}</Link> : m.display_name}</h2><p className="network-muted">{m.slug ? [m.discipline,m.city].filter(Boolean).join(" · ") : "Sin perfil público"}</p></div></header>
