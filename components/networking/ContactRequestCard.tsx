...
export default function ContactRequestCard({ item, detail = false, view = "request", contactPage = 1 }: { item: ContactRequest; detail?: boolean; view?: "request" | "contact"; contactPage?: number }) {
  ...
  return <article id={"contact-"+item.id} className={"network-card " + (view === "contact" ? "network-card--contact" : "")}>
    ...
    <p>{item.message}</p>
    {project && <section className="network-snapshot"><StatusBadge tone="info">Proyecto</StatusBadge><h3>{project.title}</h3><MetaChips labels={[project.project_type,project.city,SCHEDULES[project.shooting_schedule],ECONOMICS[project.economic_mode]]} />{project.date_window && <p className="network-muted">{project.date_window}</p>}{project.roles.length > 0 && <p>Roles: {project.roles.join(", ")}</p>}
